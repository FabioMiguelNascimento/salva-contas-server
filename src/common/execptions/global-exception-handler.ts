...
export class GlobalExceptionHandler implements ExceptionFilter {
    ...
    if (exception instanceof HttpException) {
      ...
      switch (status) {
        ...
        case 401:
          return response
            .status(401)
            .json(
              authenticationError(exception.message || 'Credenciais inválidas'),
            );
        case 403:
          return response
            .status(403)
            .json(authorizationError(exception.message || 'Acesso negado'));
        case 404:
          return response
            .status(404)
            .json(notFound(exception.message || 'Recurso não encontrado'));
        case 500:
        case 503:
        default:
          if (status === 503) {
          ...
          return response
            .status(500)
            .json(
              internalError(exception.message || 'Erro interno do servidor'),
            );
      }
    } else {
      ...
      if (this.isAIServiceUnavailable(exception)) {
        return response
          ...
          .json(
            serviceUnavailable(
              'O modelo de IA está sobrecarregado. Tente novamente mais tarde.',
            ),
          );
      }
      return response
        .status(500)
        .json(internalError('Erro interno do servidor'));
    }
  }
  ...
  private handleBadRequest(exception: HttpException, response: Response) {
    ...
    return response
      .status(400)
      .json(badRequest(exception.message || 'Requisição inválida'));
  }
  ...
  private isAIServiceUnavailable(exception: any): boolean {
    return (
      exception?.name === 'GoogleGenerativeAIFetchError' ||
      exception?.status === 503 ||
      (typeof exception?.message === 'string' &&
        exception.message.includes('model is overloaded'))
    );
  }
}
