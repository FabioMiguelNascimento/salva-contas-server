...
export class GroqGenAIProvider {
  ...
  private readonly modelPool = (
    process.env.GROQ_TRANSACTION_MODELS ||
  ...
  private readonly modelCooldownMs = Number(
    process.env.GROQ_TRANSACTION_MODEL_COOLDOWN_MS || 15000,
  );
  ...
  private buildGroqContent(input: GenerateStructuredInput): any[] {
    ...
    if (input.file && !input.file.mimetype.startsWith('image/')) {
      ...
      throw Object.assign(
        new Error('Groq provider supports image inputs only for vision.'),
        { status: 422 },
      );
    }
  ...
  async generateStructuredJson(
    ...
    if (!this.apiKey) {
      throw Object.assign(new Error('GROQ_API_KEY not configured'), {
        status: 401,
      });
    }
    ...
    for (const modelName of modelsToTry) {
      try {
        ...
        if (!text || typeof text !== 'string') {
          throw Object.assign(new Error('Groq returned empty content'), {
            status: 502,
            name: 'GroqAPIError',
          });
        }
        ...
        if (status === 429 || status === 503) {
          ...
          this.logger.warn(
            `Groq indisponivel (${modelName}) - status ${status}. Cooldown ${retryMs}ms.`,
          );
          continue;
        }
        throw error;
